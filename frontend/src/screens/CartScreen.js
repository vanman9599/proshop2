    ...
    {cartItems.length ===0 ? (
        ...
        {cartItems.map(item=> (
          <ListGroup.Item key={item.product}>
            <Row>
              ...
              <Col md={3}>
              <Form.Control as="select" value={item.qty} onChange={(e)=> dispatch(addToCart(item.product, Number(e.target.value)))} xs="auto" className="my-1" >
                      {
     ...
     <Col md={4} >
          <Card>
            ...
            <ListGroup.Item>
              <Button onClick={checkoutHandler} disabled={cartItems.length ===0} type="button" className="btn-block">
